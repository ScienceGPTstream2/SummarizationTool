    ...
    if (paragraphEval?.groundTruth && summariesByModel && Object.keys(summariesByModel).length > 0) {
      ...
      for (const [modelId, summaryText] of Object.entries(summariesByModel)) {
        rows.push({
          ...
          Relevance: "",
          Safety: "",
          "Human Eval": paragraphEval.humanScore != null ? `${paragraphEval.humanScore}%` : "",
          "Doc Parse Cost": "",
          "Extraction Cost": formatCost((fileItem as any).paragraphSummaryCost as number),
          "Eval Cost": "",
        });
      }
      ...
      rows.push({
        "Study Name": fileName,
        "LLM (Source)": getDisplayModelName((fileItem as any).paragraphSummaryModel || ""),
        Ingestion: ingestionTool,
        "System Prompt": "",
        "Prompt Template": "",
        Entity: "Paragraph Evaluation",
