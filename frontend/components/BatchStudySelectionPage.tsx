        ...
        <Card>
          ...
          <CardContent>
            <Accordion type="multiple" defaultValue={[]} className="w-full">
              ...
              {/* Cohere Models */}
              {(() => {
                const cohereModels = availableModels.filter((m) =>
                  m.provider?.toLowerCase().includes("cohere")
                );
                if (cohereModels.length === 0) return null;

                const cohereIds = cohereModels.map((m) => m.id);
                const selection = getCategorySelection(cohereIds);

                return (
                  <AccordionItem value="cohere">
                    <AccordionTrigger className="hover:no-underline">
                      <div className="flex items-center justify-between w-full pr-4">
                        <div className="flex items-center gap-2">
                    ...
                    <AccordionContent>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between pb-2 border-b">
                          <span className="text-sm text-muted-foreground">
                            {cohereModels.length} model
                            {cohereModels.length !== 1 ? "s" : ""} available
                          </span>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 text-xs"
                            onClick={() =>
                              toggleCategory(cohereIds, !selection.allSelected)
                            }
                          >
                            {selection.allSelected ? (
                              <>
