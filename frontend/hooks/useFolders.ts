...
export function useFolders() {
  ...
  const fetchFolders = useCallback(
    async (
      ...
      try {
        ...
        const data: Folder[] = await apiRequest(
          `/api/templates/folders?${params.toString()}`
        );
        setFolders(data);
        return data;
      } catch (err: any) {
  ...
  const renameFolder = useCallback(
    async (folderId: string, newName: string): Promise<Folder | null> => {
      try {
        const folder: Folder = await apiRequest(
          `/api/templates/folders/${folderId}`,
          {
            method: "PATCH",
  ...
  const deleteFolder = useCallback(
    async (folderId: string): Promise<boolean> => {
      try {
        await apiRequest(`/api/templates/folders/${folderId}`, {
          method: "DELETE",
        });
