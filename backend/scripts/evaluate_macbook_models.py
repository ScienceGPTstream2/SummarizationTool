...
class Config:
    ...
    test_document_file: Path = field(
        default_factory=lambda: Path(
            "64596011f75ffd2916b1ce50131f3d7cb36c10141e914e435fd5dc0e007b2b52_base.md"
        )
    )
    output_file: Path = field(
        default_factory=lambda: Path("model_evaluation_results.xlsx")
    )
...
class TestResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "Entity Outputs": "\n\n".join(
                [f"{k}: {v}" for k, v in self.entity_responses.items()]
            ),
            "Ground Truth": "\n\n".join(
                [f"{k}: {v}" for k, v in self.ground_truth.items()]
            ),
            "Score": f"{self.score:.2%}" if self.error is None else "N/A",
            "Latency (seconds)": (
                f"{self.total_latency_seconds:.2f}" if self.error is None else "N/A"
            ),
            "Error": self.error or "",
        }
    ...
    def parse_model_list(cls, csv_content: str) -> List[str]:
        ...
        for model in cls.EXPECTED_MODELS:
            if model.lower().replace("-", "").replace(
                ":", ""
            ) in csv_content.lower().replace("-", "").replace(":", ""):
...
class PromptLoader:
    ...
    def load(self) -> Tuple[List[EntityPrompt], Dict[str, str]]:
        ...
        for ep in entity_prompts:
            ep.prompt_with_examples = self._build_full_prompt(
                ep.prompt_with_examples, document_content
            )
    ...
    def _parse_prompt_file(
        self, content: str
    ) -> Tuple[List[EntityPrompt], Dict[str, str]]:
            ...
            if entity_line:
                ...
                if entity_name and full_prompt:
                    entity_prompts.append(
                        EntityPrompt(
                            entity_name=entity_name, prompt_with_examples=full_prompt
                        )
                    )
    ...
    def generate(self, model: str, prompt: str) -> Tuple[str, float, Optional[str]]:
        ...
        try:
            ...
            if response.ok:
                data = response.json()
                content = (
                    data.get("response") or data.get("content") or data.get("text", "")
                )
    ...
    def generate(self, results: List[TestResult]):
        ...
        for row_idx, result in enumerate(results, 2):
            ...
            ws.cell(
                row=row_idx,
                column=4,
                value=result.score if result.error is None else "N/A",
            )
            ...
            ws.cell(
                row=row_idx,
                column=5,
                value=result.total_latency_seconds if result.error is None else "N/A",
            )
...
class ModelTestRunner:
    ...
    def run(self):
        ...
        print(f"\n🚀 Starting evaluation...")
        print(
            f"   Total API calls: {total_calls} ({len(models)} models × {len(entity_prompts)} entities)"
        )
        ...
        for model_idx, model in enumerate(models, 1):
            ...
            for ep in entity_prompts:
                ...
                print(
                    f"   📤 Extracting: {entity_name}... (prompt: {prompt_len} chars, ~{prompt_tokens_approx} tokens)"
                )

                # Send request
                response, latency, error = self.client.generate(
                    model, ep.prompt_with_examples
                )
