...
class ExtractFigureContentRequest(BaseModel):
    ...
    temperature: float = Field(
        default=0.0,
        description="Sampling temperature for extraction"
    )
    system_message: Optional[str] = Field(
        default=None,
        description="Custom system message for the model"
    )
...
class FigureMetadata(BaseModel):
    ...
    caption: Optional[str] = Field(default=None, description="Figure caption if available")
    image_path: Optional[str] = Field(default=None, description="Path to figure image file")
    bounding_regions: Optional[list] = Field(default=None, description="Figure bounding regions")
    extracted_content: Optional[FigureExtractionResult] = Field(
        default=None,
        description="OCR extraction results if available"
    )
