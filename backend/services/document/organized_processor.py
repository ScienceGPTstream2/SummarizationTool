...
class OrganizedDocumentProcessor:
    ...
    async def process_document(
        ...
        if not force_reprocess:
            if await self.file_service.is_file_processed(file_hash, processor):
                ...
                markdown_content = await self.file_service.get_processed_content(file_hash, processor)
        ...
        if result["success"] and self.file_service.supabase.is_configured:
            try:
                global_file = await self.file_service.supabase.get_file_by_hash(file_hash)
                if global_file:
    ...
    async def _process_with_azure(self, file_path: str, output_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            old_conversion_dir = self.azure_service.output_base_dir / result["conversion_id"]
    ...
    async def _process_with_docling(self, file_path: str, output_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            # Copy files to new location
            old_conversion_dir = self.docling_service.output_base_dir / result["conversion_id"]
