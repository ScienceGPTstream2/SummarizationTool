...
class DocumentService:
        ...
        return {
            ...
            "processors": {
                ...
                ProcessorType.DOCLING.value: {
                    ...
                    "strengths": ["Academic papers", "Mixed content", "Fast processing", "Always available"],
    ...
    async def get_conversion_by_id(self, conversion_id: str) -> Optional[Dict[str, Any]]:
        ...
        result = await self.azure_doc_intelligence_service.get_conversion_by_id(conversion_id)
        if result:
            return result
            
        return None
    
    async def get_markdown_content(self, conversion_id: str, processor_used: Optional[str] = None) -> Optional[str]:
        """
        ...
        if processor_used:
            if processor_used == ProcessorType.AZURE_DOC_INTELLIGENCE.value:
                content = await self.azure_doc_intelligence_service.get_markdown_content(conversion_id)
                if content:
                    return content
