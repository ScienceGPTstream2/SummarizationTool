...
macbook_queue.py
================
Singleton FIFO queue for serializing all requests to Macbook-hosted LLM models.

Problem: When multiple extraction requests (e.g. 16 entities × 3 models = 48 calls)
hit the MacBook's Ollama server simultaneously, it tries to load all requested models
...
This guarantees the MacBook only handles one LLM inference at a time.
...
class MacbookRequestQueue:
    """FIFO queue with a single worker for serializing Macbook LLM requests."""
    ...
    async def _worker(self) -> None:
        ...
        print("[MacbookQueue] Worker started — processing requests one at a time")
        while True:
            try:
                ...
                print(
                    f"[MacbookQueue] Processing request #{request_num} "
                    f"(queue depth: {queue_depth} remaining)"
                )

                start = time.time()
                try:
                    # Execute the actual Macbook API call
                    result = await coro_fn(*args, **kwargs)
                    elapsed = time.time() - start
                    print(
                        f"[MacbookQueue] Request #{request_num} completed in {elapsed:.2f}s "
                    ...
                    elapsed = time.time() - start
                    print(
                        f"[MacbookQueue] Request #{request_num} failed after {elapsed:.2f}s: {exc}"
                    )
                ...
                print("[MacbookQueue] Worker cancelled")
                break
            except Exception as exc:
                print(f"[MacbookQueue] Worker error (continuing): {exc}")
    ...
    async def enqueue(
        ...
        **kwargs: Any,
        ...
        if self._started and self._worker_task is not None and self._worker_task.done():
            print("[MacbookQueue] Worker task ended unexpectedly — restarting")
            self._started = False
        ...
        queue_depth = self._queue.qsize()
        print(
            f"[MacbookQueue] Enqueued request #{self._total_enqueued} "
...
# ── Singleton accessor ───────────────────────────────────────────────────────
_global_queue: Optional[MacbookRequestQueue] = None


def get_macbook_queue() -> MacbookRequestQueue:
    """Get (or create) the global MacBook request queue singleton."""
    global _global_queue
    if _global_queue is None:
        _global_queue = MacbookRequestQueue()
