...
class SupabaseStorageService:
    ...
    def __init__(self):
        ...
        if not self.service_role_key:
            print(
                "Warning: SUPABASE_SERVICE_ROLE_KEY not configured. Supabase operations will fail."
            )
    ...
    async def delete(self, table: str, filters: Dict[str, Any]) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.delete(
                url, headers=self._get_headers(), params=filters
            )
            response.raise_for_status()
            return True
    ...
    async def get_file_by_hash(self, file_hash: str) -> Optional[Dict[str, Any]]:
        ...
        results = await self.query(
            table="global_files", filters={"file_hash": f"eq.{file_hash}"}, limit=1
        )
    ...
    async def add_user_file(
        self, user_id: str, file_id: str, nickname: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def get_user_prompts(
        self, user_id: str, prompt_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        return await self.query(
            table="user_prompts", filters=filters, order="created_at.desc"
        )
    ...
    async def update_user_prompt(
        self, prompt_id: str, user_id: str, updates: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def delete_user_prompt(self, prompt_id: str, user_id: str) -> bool:
        ...
        return await self.delete(
            "user_prompts", {"id": f"eq.{prompt_id}", "user_id": f"eq.{user_id}"}
        )
    ...
    async def get_user_settings(self, user_id: str) -> Optional[Dict[str, Any]]:
        """Get user settings."""
        results = await self.query(
            table="user_settings", filters={"user_id": f"eq.{user_id}"}, limit=1
        )
        return results[0] if results else None
