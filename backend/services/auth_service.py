...
class AuthService:
    ...
    def _save(self):
        data = {
            "whitelist": {"emails": self.whitelist},
            "users": self.users
        }
    ...
    def authenticate_user(self, email: str, password: str) -> str:
        ...
        for u in self.users:
            if u.get("email") == email:
                if bcrypt.checkpw(password.encode("utf-8"), u.get("hash").encode("utf-8")):
                    payload = {
                        "sub": email,
