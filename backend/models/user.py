...
class User(Base):
    ...
    updated_at = Column(
        DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, name="updatedAt"
    )
...
class Account(Base):
    ...
    access_token_expires_at = Column(
        DateTime, nullable=True, name="accessTokenExpiresAt"
    )
    refresh_token_expires_at = Column(
        DateTime, nullable=True, name="refreshTokenExpiresAt"
    )
