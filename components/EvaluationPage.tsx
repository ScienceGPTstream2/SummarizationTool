...
export function EvaluationPage({
  ...
  // Paragraph evaluation state (human-only, no LLM judge)
  const [paragraphEvalGroundTruth, setParagraphEvalGroundTruth] =
    useState<string>("");
  const [paragraphEvalHumanScore, setParagraphEvalHumanScore] = useState<
    number | null
  >(null);
  const [isGeneratingParagraphEval, setIsGeneratingParagraphEval] =
    useState(false);

  // Sync paragraph eval state when current file changes
  useEffect(() => {
    const evalData = (currentFile as any)?.paragraphEvaluation;
    setParagraphEvalGroundTruth(evalData?.groundTruth || "");
    setParagraphEvalHumanScore(evalData?.humanScore ?? null);
  }, [selectedFileId, files]);
  ...
  // Generate paragraph evaluation ground truth deterministically (no LLM)
  const generateParagraphEvaluation = async () => {
    const sessionId = documentData.sessionId;
    const fileId = currentFile?.fileId;
    if (!sessionId || !fileId) return;

    setIsGeneratingParagraphEval(true);
    try {
      const token = await getValidToken();
      if (!token) return;

      const entityOrder = (currentFile?.entities || []).map((e: any) => e.name);
      ...
      if (response.ok) {
        ...
        setParagraphEvalGroundTruth(data.ground_truth);
        // Update files state so BatchResultsPage picks it up
        setFiles((prev) =>
          prev.map((f) =>
            f.fileId === fileId
              ? {
                  ...
                  paragraphSummaryModel:
                    data.paragraph_model_id ||
                    (f as any).paragraphSummaryModel ||
                    "",
                  paragraphEvaluation: {
        ...
        toast.success("Paragraph evaluation ground truth generated");
      } else {
        ...
        const errMsg =
          typeof errDetail === "string"
            ...
            : "Unknown error";
        toast.error("Failed to generate ground truth: " + errMsg);
      }
    } catch (error) {
      console.error("[ParagraphEval] Error generating ground truth:", error);
      toast.error("Failed to generate paragraph evaluation ground truth");
    } finally {
      setIsGeneratingParagraphEval(false);
    }
  };

  // Save human score for paragraph evaluation (0-100 scale)
  const saveParagraphHumanScore = async (score: number | null) => {
    const paragraphModelId =
      (currentFile as any)?.paragraphSummaryModel || "";
    if (!paragraphModelId) {
      console.warn(
        "[ParagraphEval] No paragraphSummaryModel found, cannot save human score"
      );
      return;
    }
    await saveHumanScore({
      fileId: currentFile?.fileId,
      entityName: "__paragraph_summary__",
      sourceModel: paragraphModelId,
      judgeModel: "human",
      humanScore: score,
      groundTruth: paragraphEvalGroundTruth,
    });
      ...
      prev.map((f) =>
        f.fileId === currentFile?.fileId
          ? {
              ...
              paragraphEvaluation: {
                groundTruth:
                  (f as any).paragraphEvaluation?.groundTruth ||
                  paragraphEvalGroundTruth,
                humanScore: score,
              },
            }
          : f
          ...
          <div className="grid gap-6">
            ...
            {!isBatchMode && (
              <>
                ...
                {/* Paragraph Evaluation (Human-Only) - Single Mode */}
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <BarChart3 className="h-5 w-5 text-purple-500" />
                      Paragraph Evaluation (Human-Only)
                    </CardTitle>
                    <CardDescription>
                      Score the generated paragraph against a deterministic
                      ground truth compiled from extracted entities. No LLM
                      judge — human score only.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {!(currentFile as any)?.finalSummary ? (
                      <Alert>
                        <AlertDescription>
                          No paragraph generated yet. Generate a paragraph from
                          the extraction page first.
                        </AlertDescription>
                      </Alert>
                    ) : (
                      <>
                        <div>
                          <Label className="mb-1 block text-sm font-medium">
                            Generated Paragraph (Actual Output)
                          </Label>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={generateParagraphEvaluation}
                              disabled={isGeneratingParagraphEval}
                              className="h-7 text-xs"
                            >
                              {isGeneratingParagraphEval ? (
                                <>
                          ...
                          {paragraphEvalGroundTruth ? (
                            ...
                            <div className="h-28 rounded-md border border-dashed flex items-center justify-center text-sm text-muted-foreground">
                              Click "Generate Ground Truth" to compile entity
                        ...
                        {paragraphEvalGroundTruth && (
                          <div>
                            <Label
                              htmlFor="para-human-score-single"
                            ...
                            <div className="flex items-center gap-3">
                              <Input
                                id="para-human-score-single"
                                type="number"
                                min={0}
                                max={100}
                                placeholder="Enter score..."
                                value={
                                  paragraphEvalHumanScore !== null
                                    ? paragraphEvalHumanScore
                                    : ""
                                }
                                onChange={(e) => {
                                  ...
                                  setParagraphEvalHumanScore(val);
                                  saveParagraphHumanScore(val);
                                }}
                                className="w-36"
                              />
                              {paragraphEvalHumanScore !== null && (
                                <span className="text-sm text-muted-foreground">
                                  {paragraphEvalHumanScore}/100
                                </span>
                              )}
                            </div>
                          </div>
