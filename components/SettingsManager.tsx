...
export const apiKeyConfigs: Record<string, ApiKeyConfig> = {
  ...
  together_api_key: {
    ...
    description: 'API key for Llama and other open-source models via Together AI',
    placeholder: '',
...
export class SettingsManager {
  ...
  private serverConfig: { is_azure_openai_configured: boolean } = { is_azure_openai_configured: false };
  ...
  private loadSettings() {
    ...
    if (stored) {
      try {
        ...
        if (parsed && typeof parsed === 'object' && (parsed.apiKeys || parsed.customModels)) {
  ...
  getAvailableModels(): ModelConfig[] {
    const combined: ModelConfig[] = [
      ...allModels,
      ...this.customModels.map((m): ModelConfig => ({
        id: m.id,
        name: m.name,
        provider: m.provider,
    ...
    return combined.filter(model => {
      ...
      if (model.provider && model.provider.toLowerCase().includes('azure')) {
        ...
        return !!apiKey && !!endpoint && apiKey !== 'YOUR_AZURE_OPENAI_API_KEY_HERE' && endpoint !== 'YOUR_AZURE_OPENAI_ENDPOINT_HERE';
      }

      const hasApiKey = this.getApiKey(model.requiredApiKey);
      return !!hasApiKey && hasApiKey !== 'YOUR_' + model.requiredApiKey.toUpperCase() + '_HERE';
    });
  }

  isModelAvailable(modelId: string): boolean {
    const model = [...allModels, ...this.customModels].find(m => m.id === modelId);
    if (!model) return false;

    if ((model as any).provider && (model as any).provider.toLowerCase().includes('azure')) {
      if (this.serverConfig.is_azure_openai_configured) {
        return true;
      }
      const apiKey = this.getApiKey('azure_openai_api_key');
    ...
    return !!apiKey && apiKey !== 'YOUR_' + ((model as any).requiredApiKey || '').toUpperCase() + '_HERE';
  }
